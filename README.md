...
identical. `-provider auto` (the default) picks **Gemini** when a Gemini key or a
Vertex/GCP env is configured, otherwise **Anthropic**; force it with
`-provider gemini|anthropic`. Within Gemini, Vertex is used when
...
- Each provider defaults to a snappy/cheap model (`gemini-2.5-flash`,
  `claude-sonnet-5`); edit `geminiModel` in `brain_gemini.go` (→ `gemini-2.5-pro`)
  or `anthropicModel` in `brain_anthropic.go` (→ `anthropic.ModelClaudeOpus4_8`)
